...
INT wifi_hal_wps_event(wifi_wps_event_t data)
{
    ...
    if (data.event == WPS_EV_SUCCESS) {
        ...
        if (vap->vap_mode == wifi_vap_mode_sta) {
            ...
            if (ssid != NULL) {
                ...
                if (ssid->ssid && ssid->ssid_len > 0) {
                    size_t ssid_copy_len = ssid->ssid_len;
                    if (ssid_copy_len >= sizeof(vap->u.sta_info.ssid)) {
                        ssid_copy_len = sizeof(vap->u.sta_info.ssid) - 1;
                    }
                    memset(vap->u.sta_info.ssid, 0, sizeof(vap->u.sta_info.ssid));
                    memcpy(vap->u.sta_info.ssid, ssid->ssid, ssid_copy_len);
                    vap->u.sta_info.ssid[ssid_copy_len] = '\0';
                ...
                if (ssid->passphrase) {
                    ...
                    if (len >= sizeof(vap->u.sta_info.security.u.key.key)) {
                        len = sizeof(vap->u.sta_info.security.u.key.key) - 1;
                    }
                    if (len > 0) {
                        ...
                        vap->u.sta_info.security.u.key.key[len] = '\0';
        ...
        /* AP-mode WPS success: still forward to platform handler */
        {
            platform_wps_event_t platform_wps_event_fn;
            if ((platform_wps_event_fn = get_platform_wps_event_fn()) != NULL) {
                wifi_hal_dbg_print("%s:%d: platform wps event callback triggered for AP event %d\n", __func__, __LINE__, data.event);
...

    {
        platform_wps_event_t platform_wps_event_fn;
        if ((platform_wps_event_fn = get_platform_wps_event_fn()) != NULL) {
            wifi_hal_dbg_print("%s:%d: platform wps event callback triggered for event %d\n", __func__, __LINE__, data.event);
            return platform_wps_event_fn(data);
        }
    }
...
#else
    wifi_hal_error_print("%s:%d: wpa_supplicant reinit not supported on this platform\n", __func__, __LINE__);
    return RETURN_ERR;
#endif
