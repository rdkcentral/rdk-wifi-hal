...
INT wifi_hal_kickAssociatedDevice(INT ap_index, mac_address_t mac)
{
    ...
    if (memcmp(mac, bcastmac, sizeof(mac_address_t)) == 0) {
        ...
        while(tmp) {
#if defined(BANANA_PI_PORT) && (HOSTAPD_VERSION >= 211)
            wifi_drv_sta_disassoc(interface, own_addr,tmp->addr,WLAN_REASON_UNSPECIFIED,0);
#else
...
#if defined(BANANA_PI_PORT) && (HOSTAPD_VERSION >= 211)
        wifi_drv_sta_disassoc(interface, own_addr,mac,WLAN_REASON_UNSPECIFIED,0);
#else
...
void wifi_hal_disassoc(int vap_index, int status, uint8_t *mac)
{
    ...
    pthread_mutex_unlock(&g_wifi_hal.hapd_lock);
#if defined(BANANA_PI_PORT) && (HOSTAPD_VERSION >= 211)
    wifi_drv_sta_disassoc(interface, own_addr, mac, status,0);
#else
