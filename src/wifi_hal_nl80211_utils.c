...
{0, 2,  "wl2.7",   "brlan114", 114,   22,     "mesh_backhaul_6g"},
{0, 2,  "wl2",     "",         0,     23,     "mesh_sta_6g"},
#else /* RDKB_ONE_WIFI_2_RADIO_SUPPORT */
#endif
#endif
...
#ifdef RDKB_ONE_WIFI_PROD
#define NUM_RADIOS 3
unsigned int pcie_index[NUM_RADIOS];

static bool parse_wiphy_band_mapping(FILE *fp) {
    char line[LINE_MAX];
    int curr_phy_idx;
    bool in_wiphy = false;
        ...
        if (in_wiphy) {
            ...
            if (strncmp(trimmed, "Band ", 5) == 0) {
                ...
                if (sscanf(trimmed, "Band %d:", &band_num) == 1) {
                    printf("phy%d-band%d\n", curr_phy_idx, band_num);
                    if (curr_phy_idx < NUM_RADIOS)
                        pcie_index[curr_phy_idx] = ((band_num == 4) ? 2 : (band_num-1));
                    else {
                        wifi_hal_error_print("%s:%d: Recieved phy_index %d > %d", __func__, __LINE__, curr_phy_idx, NUM_RADIOS);
                        return false;
                    }
                }
            }
            // If a new Wiphy begins, reset
            if (strstr(line, "Wiphy ") == line) {
                if (sscanf(line, "Wiphy phy%d", &curr_phy_idx) == 1) {
                    in_wiphy = true;
                }
            }
...
static void remap_phy_index(wifi_interface_name_idex_map_t *map, int map_size, const unsigned int *pcie_index, int pcie_size)
{
...
void remap_wifi_interface_name_idex_map() {
    ...
    int i;
    fp = popen("iw list", "r");
    if (parse_wiphy_band_mapping(fp)) {
        remap_phy_index(interface_index_map, sizeof(interface_index_map)/sizeof(interface_index_map[0]), pcie_index, NUM_RADIOS);
    }
    wifi_hal_error_print("%s:%d\n", __func__, __LINE__);
    for (i = 0; i < sizeof(interface_index_map)/sizeof(interface_index_map[0]); ++i) {
        wifi_hal_error_print("%d %s\n", interface_index_map[i].phy_index, interface_index_map[i].interface_name);
    }
    wifi_hal_error_print("%s:%d\n", __func__, __LINE__);
    pclose(fp);
}
