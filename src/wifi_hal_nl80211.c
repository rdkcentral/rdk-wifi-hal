...
int nl80211_add_vlan(const char *if_name, int vid, const char *vlan_if_name)
{
	...
	wifi_hal_error_print(MSG_ERROR, "VLAN: vlan_add(if_name=%s, vid=%d, "
		   "vlan_if_name=%s)\n", if_name, vid, vlan_if_name);

    	if ((os_strlen(if_name) + 1) > IFNAMSIZ) {
		wifi_hal_error_print(MSG_ERROR, "Interface name too long: '%s'",
			   if_name);
		return -1;
	}

	if ((os_strlen(vlan_if_name) + 1) > IFNAMSIZ) {
		wifi_hal_error_print(MSG_ERROR, "VLAN Interface name too long: '%s'",
			   vlan_if_name);
		return -1;
	}
	...
	if (!handle) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to open netlink socket");
		goto vlan_add_error;
	}

	err = nl_connect(handle, NETLINK_ROUTE);
	if (err < 0) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to connect to netlink: %s",
			   nl_geterror(err));
		goto vlan_add_error;
	}
	...
	if (err < 0) {
		/* link does not exist */
		wifi_hal_error_print(MSG_ERROR, "VLAN: interface %s does not exist",
			   if_name);
		goto vlan_add_error;
	}
	...
	if (err >= 0) {
		...
		rtnl_link_put(rlink);
		rlink = NULL;
		wifi_hal_error_print(MSG_ERROR, "VLAN: interface %s already exists",
			   vlan_if_name);
		ret = 1;
		goto vlan_add_error;
	}
	...
	if (!rlink) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to allocate new link");
		goto vlan_add_error;
	}

	err = rtnl_link_set_type(rlink, "vlan");
	if (err < 0) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to set link type: %s",
			   nl_geterror(err));
		goto vlan_add_error;
	}
	...
	if (err < 0) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to set link vlan id: %s",
			   nl_geterror(err));
		goto vlan_add_error;
	}
	...
	if (err < 0) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to create link %s for "
			   "vlan %d on %s (%d): %s",
...
int nl80211_remove_vlan(const char *if_name)
{
	...
	wifi_hal_error_print(MSG_ERROR, "VLAN: vlan_rem(if_name=%s)", if_name);
	...
	if (err < 0) {
		/* link does not exist */
		wifi_hal_error_print(MSG_ERROR, "VLAN: interface %s does not exists",
			   if_name);
		goto vlan_rem_error;
	}
	...
	if (err < 0) {
		wifi_hal_error_print(MSG_ERROR, "VLAN: failed to remove link %s: %s",
