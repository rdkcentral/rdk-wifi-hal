...
bm_sta_list_t *steering_add_stalist(wifi_interface_info_t *interface, char *ssid, mac_address_t client_mac, uint8_t type)
{
    ...
    if (bm_client_info == NULL) {
        ...
        if (hash_map_put(interface->bm_sta_map, key, bm_client_info)) {
            free(bm_client_info);
...
static rate_limit_entry_t *wifi_hal_rate_limit_entry_get(mac_address_t mac)
{
    ...
    entry->last_activity = time_now;
    if (hash_map_put(g_wifi_hal.mgt_frame_rate_limit_hashmap, mac_str, entry)) {
        free(entry);
             ...
             void *valid_finish_data)
{
    ...
    if (!nl_info) {
        if ((nl_info = create_nl80211_socket())) {
            if (hash_map_put(g_wifi_hal.netlink_socket_map, thread_id, nl_info)) {
...
int interface_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_WIPHY]) {
        ...
        if (radio != NULL && tb[NL80211_ATTR_IFNAME]) {
            ...
            if (hash_map_put(radio->interface_map, interface->name, interface)) {
...
int init_nl80211()
{
    ...
    if (hash_map_put(g_wifi_hal.netlink_socket_map, thread_id, core_thread_socket)) {
...
static int scan_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (scan_info_ap->ssid[0] != '\0') {
        ...
        if (scan_info == NULL) {
            ...
            if (hash_map_put(interface->scan_info_map, key, scan_info)) {
                pthread_mutex_unlock(&interface->scan_info_mutex);
                free(scan_info);
                free(scan_info_ap);
    ...
    pthread_mutex_lock(&interface->scan_info_ap_mutex);
    if (hash_map_put(interface->scan_info_ap_map[0], key, scan_info_ap)) {
        pthread_mutex_unlock(&interface->scan_info_ap_mutex);
