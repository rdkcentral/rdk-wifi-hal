...
int nl80211_create_bridge(const char *if_name, const char *br_name)
{
    ...
    if (strcmp(br_name,"brww0")) {
        ...
        ret = system(cmd);
        if (ret != 0) {
        wifi_util_dbg_print(WIFI_CTRL, "%s:%d SREESH Failed to set bridge MAC, ret=%d\n",
            __func__, __LINE__, ret);
        } else {
        wifi_util_info_print(WIFI_CTRL, "%s:%d SREESH Successfully set bridge MAC to D4:E2:CB:9D:47:09\n",
            __func__, __LINE__);
        }
    }
...
#endif
    wifi_hal_info_print("%s:%d: SREESH Using interface %s\n", __func__, __LINE__, ifname);
    memset(&sockaddr, 0, sizeof(struct sockaddr_ll));
    sockaddr.sll_family   = AF_PACKET;
    sockaddr.sll_ifindex  = if_nametoindex(ifname);
