...
int nl80211_connect_sta(wifi_interface_info_t *interface)
{
    ...
    if (bh_rsn && (wpa_parse_wpa_ie_rsn((const u8*)bh_rsn, 
                                         bh_rsn->length+sizeof(ieee80211_tlv_t), &data) == 0)) {
...
static int scan_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (vap->vap_mode == wifi_vap_mode_sta) {
        if (strcmp(scan_info_ap->ssid, vap->u.sta_info.ssid) == 0) {
            ...
            if (beacon_ies && (beacon_ie->buff = (u8*)realloc(beacon_ie->buff, beacon_ie_len)) != NULL) {
