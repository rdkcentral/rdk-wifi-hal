...
static int scan_results_handler(struct nl_msg *msg, void *arg)
{
    ...
    bool is_wildcard_ssid = (strlen(interface->vap_info.u.sta_info.ssid) == 0);
    ...
    *finish_data->err = 0;
    ...
    if (interface->vap_info.vap_mode == wifi_vap_mode_sta) {
        ...
        while (scan_info != NULL) {
            if (strcmp(scan_info->ssid, interface->vap_info.u.sta_info.ssid) == 0 || is_wildcard_ssid){
#if defined(_PLATFORM_BANANAPI_R4_)
...
static int scan_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (vap->vap_mode == wifi_vap_mode_sta) {
        ...
        if (strcmp(scan_info_ap->ssid, vap->u.sta_info.ssid) == 0 && strlen(vap->u.sta_info.ssid) > 0) {
    ...
    if (ssid[0] != '\0') {
        wifi_hal_stats_dbg_print("%s:%d: [SCAN] found bss:%s rssi:%d ssid:%s on freq:%d \n", __func__, __LINE__,
                to_mac_str(bssid, bssid_str), scan_info_ap->rssi, scan_info_ap->ssid, scan_info_ap->freq);
