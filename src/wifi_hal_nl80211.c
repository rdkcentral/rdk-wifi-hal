...
#ifdef NL80211_ACL
#ifndef WLAN_STATUS_DENIED_INSUFFICIENT_BANDWIDTH
...
int process_frame_mgmt(wifi_interface_info_t *interface, struct ieee80211_mgmt *mgmt, u16 reason, int sig_dbm, int phy_rate, unsigned int len, unsigned int recv_freq) {
    ...
    case WLAN_FC_STYPE_AUTH:
        ...
        if (is_core_acl_drop_mgmt_frame(interface, sta)) {
            wifi_hal_dbg_print("%s:%d: STA rejected by ACL; sending auth rejection frame\n", __func__, __LINE__);
            if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.auth)) {
                wifi_hal_error_print("%s:%d: interface:%s auth frame too short (len=%u), cannot send rejection\n",
                    __func__, __LINE__, interface->name, len);
            } else {
...
#ifdef NL80211_ACL
        if (is_core_acl_drop_mgmt_frame(interface, sta)) {
            wifi_hal_dbg_print("%s:%d: STA rejected by ACL; sending assoc rejection response\n", __func__, __LINE__);
...
#ifdef NL80211_ACL
        if (is_core_acl_drop_mgmt_frame(interface, sta)) {
            wifi_hal_dbg_print("%s:%d: STA rejected by ACL; sending reassoc rejection response\n", __func__, __LINE__);
