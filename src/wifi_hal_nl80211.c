...
#endif
{
    ...
    if (data_len > sizeof(buff) - sizeof(struct ieee8023_hdr)) {
        wifi_hal_error_print("%s:%d: eapol payload too large for tx buffer, len=%zu\n",
    ...
    if ((access(ONEWIFI_TESTSUITE_TMPFILE, R_OK)) == 0) {
        ...
        if (fd_c > 0) {
            ...
            unsigned int payload_len = (unsigned int)(data_len + sizeof(struct ieee8023_hdr));
            unsigned int len = payload_len + sizeof(eapol_qos_info) + sizeof(llc_info);
            if (len > sizeof(c_buff) - (3 * sizeof(unsigned int) + 2 * ETH_ALEN)) {
                wifi_hal_error_print("%s:%d: eapol payload too large for emulator char-dev buffer, len=%zu\n",
            ...
            memcpy(t_buff, data, payload_len);
