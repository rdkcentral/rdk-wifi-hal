...
int wifi_drv_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    if (nla_put(msg, NL80211_ATTR_BEACON_HEAD, params->head_len, params->head) ||
        nla_put(msg, NL80211_ATTR_BEACON_TAIL, params->tail_len, params->tail)) {
        wifi_hal_error_print("%s:%d: Failed to set beacon data\n", __func__, __LINE__);
        nlmsg_free(msg);
        return -1;
    }
    if (params->beacon_int > 0) {
        if (nla_put_u32(msg, NL80211_ATTR_BEACON_INTERVAL, params->beacon_int)) {
    ...
    if (params->dtim_period > 0) {
        if (nla_put_u32(msg, NL80211_ATTR_DTIM_PERIOD, params->dtim_period)) {
    ...
    if (num_suites) {
        if (nla_put(msg, NL80211_ATTR_AKM_SUITES, num_suites * sizeof(u32), suites)) {
    ...
    if (params->key_mgmt_suites & WPA_KEY_MGMT_IEEE8021X_NO_WPA &&
        ...
        if (nla_put_u16(msg, NL80211_ATTR_CONTROL_PORT_ETHERTYPE, ETH_P_PAE) ||
            nla_put_flag(msg, NL80211_ATTR_CONTROL_PORT_NO_ENCRYPT)) {
    ...
    if (drv->device_ap_sme && (params->key_mgmt_suites & WPA_KEY_MGMT_SAE)) {
        if (nla_put_flag(msg, NL80211_ATTR_EXTERNAL_AUTH_SUPPORT)) {
